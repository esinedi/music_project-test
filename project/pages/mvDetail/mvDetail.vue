		...
		<view class="mvDetailFooter">
			<view class="mvDetailFooterMain">
				<view class="mvDetailFooterMainName">
					<view class="mvDetailFooterMainNameP1" style="margin-bottom: 10px;">
						@{{mvData.artistName}}
					</view>
...
<script>
	export default {
		...
		onLoad:function(options){
			console.log(options)
		  this.mvid = JSON.parse(options.mvid)
		  // 获取用户coolie
		  uni.getStorage({
		  	key:"user",
		  	success: (res) => {
		  ...
		  // 获取用户评论信息
		  uni.request({
		      url: this.$Url + '/comment/mv?id=' + this.mvid,
		...
		onShow(){
			// 获取mv地址
			uni.request({
			    url: this.$Url + '/mv/url?id=' + this.mvid,
			    dataType: 'json', //默认 json格式
			    method: 'GET', //请求方式
				withCredentials:true,
			    success: (res) => {
			...
			// 获取mv数据
			uni.request({
			    url: this.$Url + '/mv/detail?mvid=' + this.mvid,
			    dataType: 'json', //默认 json格式
			    method: 'GET', //请求方式
				withCredentials:true,
			    success: (res) => {
			...
			// 获取mv点赞数，评论数
			uni.request({
			    url: this.$Url + '/mv/detail/info?mvid=' + this.mvid,
			    dataType: 'json', //默认 json格式
			    method: 'GET', //请求方式
				withCredentials:true,
			    success: (res) => {
